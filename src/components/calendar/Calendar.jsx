...
function CalendarPage({currentDay, setCurrentDay}) {
  ...
  const [daliyMealPlan, setDaliyMealPlan] = useState(['apple', 'banana', 'orange']);
  const [daliyWorkoutPlan, setDaliyWorkoutPlan] = useState(['Biceps', 'Chest', 'Glutes']);
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:3000/monthly-meals?year=${year}&month=${currMonth}`)
    .then(({data}) => {
      // console.log(data);
      data.forEach(({date, calories, category}) => {
        const d = new Date(date);
        const parsedDate = `${monthNames[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`
