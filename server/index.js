...
app.get('/login', (req, res) =>{
  db.query('SELECT user_id FROM users WHERE username = $1 AND password = $2', [req.query.username, req.query.password]).then((data, err) => {
...
app.post('/new-user', (req, res) => {
  ...
  db.query('INSERT INTO users (username, password, age, height_feet, height_inches, weight, goal_weight, goal_date, calorie_goal) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [req.body.username, req.body.password, req.body.age, req.body.height_feet, req.body.height_inches, req.body.weight, req.body.goal_weight, req.body.goal_date, req.body.calories]).then(() => {
...
app.post('/new-exercise', (req, res) => {
  db.query('INSERT INTO exercises (exercise_detail_id, user_id, date, weight, sets, reps, duration, intensity, calories_burned) VALUES ($1, $2, $3,$4,$5,$6,$7,$8, $9)', [req.body.exerciseDetail, req.body.userId, req.body.date, req.body.weight, req.body.sets, req.body.reps, req.body.duration, req.body.intensity, req.body.caloriesBurned]).then(() => {
...
app.get('/nutrition', (req, res) => {
  ...
  axios.get('https://api.api-ninjas.com/v1/nutrition/', {
          ...
          db.query('INSERT INTO food (user_id, name, date, category, calories, protein, description, quantity) VALUES ($1, $2, $3, $4, $5, $6, $7, $8 )', [req.query.userId, foody.name, req.query.date, req.query.category, foody.calories, foody.protein_g, req.query.description, foody.serving_size_g]).then(() =>{
...
app.put('/edit-meal', (req, res) => {
  db.query('UPDATE food SET quantity = $2 WHERE id= $1', [req.body.foodId, req.body.quantity] ).then(() => {
...
app.get('/monthly-calories', (req, res) => {
  db.query('SELECT date, calories FROM food WHERE EXTRACT(MONTH FROM date) = $1 AND EXTRACT (YEAR FROM date) = $2 AND user_id = $3', [req.query.month, req.query.year, req.query.userId]).then((calories) => {
...
app.get('/monthly-workout', (req, res) => {
  db.query('SELECT * FROM exercises WHERE EXTRACT(MONTH FROM date) = $1 AND EXTRACT (YEAR FROM date) = $2 AND user_id = $3', [req.query.month, req.query.year, req.query.userId]).then((workouts) => {
    res.send(workouts.rows)
  })
})
...
app.get('/training', (req, res) => {
  db.query('SELECT type FROM exercise_details INNER JOIN exercises ON exercises.exercise_detail_id = exercise_details.exercise_detail_id WHERE exercises.exercise_id = $1', [req.query.exerciseId]).then((training)=> {
